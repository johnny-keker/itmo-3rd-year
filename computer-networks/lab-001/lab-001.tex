...
\newcommand{\bandwidthEntry}[1]{
  ...
  Верхняя граница частот в передаваемом сообщении & \py{max_mult * f_0[#1]} Гц \\
  Нижняя граница частот в передаваемом сообщении & \py{f_0[#1] / max_count} Гц \\
  Необходимая полоса пропускания & \py{f_0[#1] / max_count} : \py{max_mult * f_0[#1]} Гц \\
}
